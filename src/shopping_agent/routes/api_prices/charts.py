...
async def product_price_history_batch(
    ...
    ph_rows = (await session.execute(
        select(PriceHistory.product_id, sqlfunc.date(PriceHistory.recorded_at), sqlfunc.avg(PriceHistory.price))
        .where(PriceHistory.product_id.in_(product_ids))
        .group_by(PriceHistory.product_id, sqlfunc.date(PriceHistory.recorded_at))
        .order_by(PriceHistory.product_id, sqlfunc.date(PriceHistory.recorded_at))
    )).all()
    ...
    for pid, dt, price in ph_rows:
        rows_by_product[pid].append((dt, price))
