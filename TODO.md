...
`ConsumptionPrediction` rows already carry a `user_id`, but the logic that decides which products get predictions (and at what confidence threshold) is shared config, not per-user. The `user_product_preferences` table now has `exclude_from_predictions` and the hide scoping is fixed, but the prediction refresh logic doesn't yet honour `exclude_from_predictions`, and there are no "always include" / "never include" list flags. Wire `exclude_from_predictions` into `refresh_predictions()` and add the list inclusion flags.
